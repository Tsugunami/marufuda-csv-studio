...
## 対応するテンプレート一覧（ラベル屋さん対応ラベル調査済み）

| # | ラベルサイズ | 配置 | 面数 | フォーマットコード | 代表品番 |
|---|---|---|---|---|---|
| ① | 30×30mm | 5列×8行 | 40面 | F40A4-1 | 31553 |
| ② | 20×20mm | 7列×10行 | 70面 | F70A4-1 | 31555 |
| ③ | 35×12mm | 5列×19行 | 95面 | F95A4-1 | 28790 |
| ④ | 35×8mm | 5列×25行 | 125面 | F125A4-1 | 72125 |
| ⑤ | 20×8mm | 8列×25行 | 200面 | F200A4-1 | 72200 |

> 実際のalym出力では、テンプレートは **「シート数が少ないもの」「マット紙用」** を基本とする。
> ただし行列数とサイズが合っていればどの品番のテンプレートでも読み込み可能なため、
> 本ツールでは **フォーマットコードごとに1つのテンプレートJSON** を用意し、
> blockCols×blockRows のマッチングで選択する。
